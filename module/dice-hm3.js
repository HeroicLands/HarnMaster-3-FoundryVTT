...
export class DiceHM3 {
    ...
    static async d100StdRoll(rollData) {
        ...
        const renderedNotes =
            rollData.notes ? utility.stringReplacer(rollData.notes, notesData) : "";
    ...
    static async d100StdDialog(dialogOptions) {
        // Render modal dialog
        let dlgTemplate =
            dialogOptions.template || "systems/hm3/templates/dialog/standard-test-dialog.html";
        let dialogData = {
        ...
        return foundry.applications.api.DialogV2.prompt({
            ...
            ok: {
                ...
                callback: (event, button) => {
                    const formModifier = button.form.modifier.value;
    ...
    static async injuryRoll(rollData) {
        ...
        if (game.settings.get("hm3", "combatAudio")) {
            foundry.audio.AudioHelper.play(
                { src: "systems/hm3/audio/grunt1.ogg", autoplay: true, loop: false },
                true,
            );
        }
        return chatTemplateData;
    ...
    static async injuryDialog(dialogOptions) {
        ...
        // Render modal dialog
        let dlgTemplate =
            dialogOptions.template || "systems/hm3/templates/dialog/injury-dialog.html";
        let dialogData = {
            aim: "mid",
            location: "Random",
            impact: 0,
        ...
        return foundry.applications.api.DialogV2.prompt({
            ...
            ok: {
                ...
                callback: (event, button) => {
                    ...
                    const formAddToCharSheet =
                        dialogData.askRecordInjury ?
    ...
    static _calcInjury(location, impact, aspect, addToCharSheet, aim, dialogOptions) {
        ...
        switch (result.injuryLevelText) {
            ...
            case "G4":
                ...
                result.isAmputate =
                    enableAmputate && armorLocationData.isAmputate && aspect === "Edged";
                break;
        ...
        if (armorLocationData.isFumble) {
            ...
            result.isFumbleRoll =
                enableLimbInjuries || (!result.isFumble && result.injuryLevel >= 2);
        }

        // Optional Rule - Limb Injuries (Combat 14)
        if (armorLocationData.isStumble) {
            result.isStumble = enableLimbInjuries && result.injuryLevel >= 4;
            result.isStumbleRoll =
                enableLimbInjuries || (!result.isStumble && result.injuryLevel >= 2);
        }
    ...
    static async damageRoll(rollData) {
        ...
        const totalImpact =
            weapon.aspects[roll.chosenAspect] + roll.addlWeaponImpact + roll.rollObj.total;
            ...
            ok: {
                ...
                callback: async (event, button) => {
                    ...
                    let result = {
                        ...
                        modifiedTarget:
                            Number(dialogOptions.target) + rangeModifier + formAddlModifier,
                        isSuccess: roll.isSuccess,
                        isCritical: roll.isCritical,
                        description: roll.description,
                        rollObj: roll.rollObj,
    ...
    static async missileDamageRoll(rollData) {
        ...
        const totalImpact =
            Number(rangeImpact) + Number(roll.addlImpact) + Number(roll.rollObj.total);
