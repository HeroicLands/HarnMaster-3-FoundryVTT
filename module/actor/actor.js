...
export class HarnMasterActor extends Actor {
    ...
    static async createDialog(data = {}, { parent = null, pack = null, types, ...options } = {}) {
        ...
        // Collect data
        const documentTypes = this.TYPES.filter(
            (t) => t !== CONST.BASE_DOCUMENT_TYPE && types?.includes(t) !== false,
        );
        let collection;
        if (!parent) {
        ...
        return foundry.applications.api.DialogV2.prompt({
            ...
            render: (event, dialog) => {
                ...
                dialog.element.querySelector('[name="type"]').addEventListener("change", (e) => {
                    dialog.element.querySelector('[name="name"]').placeholder =
                        this.implementation.defaultName({ type: e.target.value, parent, pack });
                });
            },
            rejectClose: false,
            ok: {
    ...
    async _preCreate(createData, options, user) {
        ...
        if (createData.type === "character") {
            ...
            // Add standard skills
            await HarnMasterActor.addItemsFromPack(
    ...
    prepareBaseData() {
        ...
        if (this.type === "container") {
            ...
            actorData.capacity.pct = Math.round(
                ((actorData.capacity.max - actorData.capacity.value) /
                    (actorData.capacity.max || 1)) *
                    100,
            );
    ...
    prepareDerivedData() {
        ...
        actorData.move.effective = Math.max(
            eph.move -
                (actorData.move.base < 25 ?
                    actorData.physicalPenalty
                :   actorData.physicalPenalty * 5),
            0,
        );
    ...
    _calcGearWeightTotals() {
                ...
                if (cid && cid != "on-person") {
                    ...
                    if (container) {
                        container.system.capacity.value =
                            Math.round(
                                (container.system.capacity.value + tempWeight + Number.EPSILON) *
                                    100,
                            ) / 100;
                    } else {
        ...
        eph.totalMiscGearWeight =
            Math.round((eph.totalMiscGearWeight + Number.EPSILON) * 100) / 100;

        eph.totalGearWeight =
            eph.totalWeaponWeight +
            eph.totalMissileWeight +
            eph.totalArmorWeight +
            eph.totalMiscGearWeight;
    ...
    _setupEffectiveAbilities(actorData) {
        ...
        actorData.abilities.comeliness.effective = Math.max(
            Math.round(eph.comeliness + Number.EPSILON),
            0,
        );
        actorData.abilities.morality.effective = Math.max(
            Math.round(eph.morality + Number.EPSILON),
            0,
        );
    }
    ...
    _setRitualInvocations(diety, sb, ml, eml) {
        ...
        this.items.forEach((it) => {
            ...
            if (
                it.type === "invocation" &&
                itemData.diety &&
                itemData.diety.toLowerCase() === lcDiety
            ) {
    ...
    static chatListeners(html) {
        ...
        HarnMasterActor._chatCardHandler ??=
            HarnMasterActor._onChatCardAction.bind(HarnMasterActor);
    ...
    static async _onChatCardAction(event) {
        ...
        if (button.dataset.atkTokenId) {
            ...
            if (!atkToken) {
                console.warn(
                    `HM3 | Action=${action}; Cannot find attack token ${button.dataset.atkTokenId}`,
                );
                button.disabled = false;
                return null;
            }
        ...
        if (button.dataset.defTokenId) {
            ...
            if (!defToken) {
                console.warn(
                    `HM3 | Action=${action}; Cannot find defense token ${button.dataset.defTokenId}`,
                );
                button.disabled = false;
                return null;
            }
            ...
            const emlChanges = e.changes.filter((chg) => {
                if (chg.key === "system.eph.itemEMLMod") {
                    ...
                    return Array.from(ownedItems).some(
                        (i) => i.name === skillName && (i.type === "skill" || i.type === "psionic"),
                    );
                } else {
                    return false;
                }
                ...
                allChanges.map((c) => {
                    ...
                    switch (c.key) {
                        case "system.eph.itemAMLMod":
                            ...
                            if (!c.item)
                                c.item = this.itemTypes.missilegear.find(
                                    (it) => it.name === itemName,
                                );
                            break;
    ...
    applySkillTypeActiveEffect(skill) {
        ...
        const changes = this.effects.reduce((chgs, e) => {
            ...
            const skillChanges = e.changes.filter(
                (chg) =>
                    (chg.key === "system.eph.commSkillsMod" &&
                        skillData.type === "Communication") ||
...
*/
    ...
    async runCustomMacro(rollInput) {
        ...
        const rollResult = {
            ...
            result:
                rollInput.isSuccess ?
                    rollInput.isCritical ?
                        "CS"
                    :   "MS"
                : rollInput.isCritical ? "CF"
                : "MF",
    ...
    static calcShockIndex(actor) {
        const data = actor.system;
        data.shockIndex.value = HarnMasterActor.normProb(
            data.endurance,
