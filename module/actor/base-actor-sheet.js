...
export class HarnMasterBaseActorSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
    ...
    async _onDropItem(event, droppedItem) {
        ...
        if (
            ...
            (!droppedItem.parent.isToken &&
                !this.actor.isToken &&
                droppedItem.parent.id === this.actor.id)
        ) {
        ...
        if (droppedItem.type === "containergear")
            return await this._moveContainer(event, droppedItem);
    ...
    async _moveContainer(event, item) {
        ...
        if (!item.parent) {
            ui.notifications.warn(
                `Error accessing actor where container is coming from, move aborted`,
            );
    ...
    async _moveItems(item, moveQuantity) {
        ...
        let result = this.actor.items.find(
            (it) => it.type === sourceType && it.name === sourceName,
        );
    ...
    _clickHandlers() {
        ...
        return {
            ...
            ".skill-roll": (ev, target) =>
                macros.skillRoll(itemOf(target)?.uuid, fastForward(ev), actor),
            ".spell-roll": (ev, target) =>
                macros.castSpellRoll(itemOf(target)?.uuid, fastForward(ev), actor),
            ".invocation-roll": (ev, target) =>
                macros.invokeRitualRoll(itemOf(target)?.uuid, fastForward(ev), actor),
            ".psionic-roll": (ev, target) =>
                macros.usePsionicRoll(itemOf(target)?.uuid, fastForward(ev), actor),

            ".ability-d6-roll": (ev, target) =>
                macros.testAbilityD6Roll(target.dataset.ability, fastForward(ev), actor),
            ".ability-d100-roll": (ev, target) =>
                macros.testAbilityD100Roll(target.dataset.ability, fastForward(ev), actor),

            ".weapon-damage-roll": (ev, target) =>
                macros.weaponDamageRoll(itemOf(target)?.uuid, target.dataset.aspect, actor),
            ".missile-damage-roll": (ev, target) =>
                macros.missileDamageRoll(itemOf(target)?.uuid, target.dataset.range, actor),
            ...
            ".weapon-attack-roll": (ev, target) =>
                macros.weaponAttackRoll(itemOf(target)?.uuid, fastForward(ev), actor),
            ".weapon-defend-roll": (ev, target) =>
                macros.weaponDefendRoll(itemOf(target)?.uuid, fastForward(ev), actor),
            ".missile-attack-roll": (ev, target) =>
                macros.missileAttackRoll(itemOf(target)?.uuid, actor),

            ".injury-roll": () => macros.injuryRoll(actor),
            ".healing-roll": (ev, target) =>
                macros.healingRoll(itemOf(target)?.uuid, fastForward(ev), actor),
    ...
    async _onItemDelete(event, target) {
        ...
        if (itemId) {
            ...
            if (!item) {
                console.error(
                    `HM3 | Delete aborted, item ${itemId} in actor ${this.actor.name} was not found.`,
                );
                return;
            }
            ...
            if (item.type === "containergear") {
                ...
                content =
                    "<p>WARNING: All items in this container will be deleted as well!</p><p>Are you sure?</p>";
            } else {
            ...
            if (agree) {
                ...
                if (item.type === "containergear") {
                    this.actor.items.forEach((it) => {
                        if (it.type.endsWith("gear") && it.system.container === itemId)
                            deleteItems.push(it.id);
                    });
                }
        ...
        switch (item.type) {
            case "skill":
                ...
                if (!data.skillBase.formula) {
                    ...
                    updateData["system.skillBase.isFormulaValid"] =
                        otherData.skillBase.isFormulaValid;
                }
                break;
    ...
    async _onItemCreate(event, target) {
        ...
        if (dataset.type === "skill" && dataset.skilltype) {
            name = utility.createUniqueName(
                `New ${dataset.skilltype} Skill`,
                this.actor.itemTypes.skill,
            );
        } else if (dataset.type == "trait" && dataset.traittype) {
            name = utility.createUniqueName(
                `New ${dataset.traittype} Trait`,
                this.actor.itemTypes.trait,
            );
            ...
            switch (dataset.type) {
                case "armorlocation":
                    name = utility.createUniqueName(
                        "New Location",
                        this.actor.itemTypes.armorlocation,
                    );
                    break;
                ...
                case "invocation":
                    name = utility.createUniqueName(
                        "New Invocation",
                        this.actor.itemTypes.invocation,
                    );
                    break;
        ...
        return foundry.applications.api.DialogV2.prompt({
            ...
            ok: {
                ...
                callback: async (event, button) => {
                    ...
                    else if (dataset.type === "trait")
                        updateData["system.type"] = dataset.traittype;
                    else if (dataset.type.endsWith("gear"))
                        updateData["system.container"] = dataset.containerId;
                    else if (dataset.type === "spell")
                        updateData["system.convocation"] = extraValue;
    ...
    async _onMoreInfo(event, target) {
        ...
        const helpJournal = await game.packs
            .find((p) => p.collection === `hm3.system-help`)
            .getDocuments();
