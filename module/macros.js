...
async function handleItemMacro(data, slot) {
    ...
    return await applyMacro(title, `await game.hm3.macros.${cmdSuffix}`, slot, item.img, {
        "hm3.itemMacro": false,
    });
}
...
export async function testAbilityD100Roll(
    ability,
    noDialog = false,
    myActor = null,
    multiplier = 5,
...
export async function changeFatigue(newValue, myActor = null) {
    ...
    if (/^\s*[+-]/.test(newValue)) {
        ...
        if (!isNaN(changeValue))
            updateData["system.fatigue"] = Math.max(
                actorInfo.actor.system.fatigue + changeValue,
                0,
            );
    } else {
...
export async function changeMissileQuanity(missileName, newValue, myActor = null) {
    ...
    if (/^\s*[+-]/.test(newValue)) {
        ...
        const changeValue = parseInt(newValue, 10);
        if (!isNaN(changeValue))
            updateData["system.quantity"] = Math.max(missile.system.quantity + changeValue, 0);
    } else {
        const value = parseInt(newValue, 10);
...
export async function setSkillDevelopmentFlag(skillName, myActor = null) {
    ...
    let speaker =
        myActor instanceof Actor ?
            ChatMessage.getSpeaker({ actor: myActor })
    ...
    if (!skill) {
        ui.notifications.warn(
            `${skillName} could not be found in the list of skills for ${actor.name}.`,
        );
        return null;
    }
    ...
    if (!actor.isOwner) {
        ui.notifications.warn(
            `You are not an owner of ${actor.name}, so you may not set the skill development flag.`,
        );
        return null;
    }
...
export async function missileAttack(
    itemName = null,
    noDialog = false,
    myToken = null,
    forceAllow = false,
